...
add_task(async function test_no_network_requests() {
  ...
  let mhtmlFile = await saveMHTMLToFile(mhtmlContent, "network_test.mhtml");
  ...
  try {
    ...
    let externalRequests = requests.filter(
      req => req.url.startsWith("https://") || req.url.startsWith("https://")
    );
...
add_task(async function test_csp_blocks_network() {
  ...
  try {
    ...
    for (let img of imageStatus) {
      if (img.src.startsWith("https://") || img.src.startsWith("https://")) {
        is(img.naturalWidth, 0, `External image should not load: ${img.src}`);
      }
    }
...
<body>
  ...
  let mhtmlContent = createMHTMLWithExternalRefs();
  let mhtmlFile = await saveMHTMLToFile(mhtmlContent, "network_compare.mhtml");
