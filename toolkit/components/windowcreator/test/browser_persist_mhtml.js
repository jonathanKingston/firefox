...
add_task(async function test_mhtml_basic() {
  ...
  // Scripts may be present in saved MHTML but should not execute when loaded.
  // Noscript content may be present but should not render when loaded.
...
add_task(async function test_mhtml_drops_noscript() {
  info("Testing that noscript elements are dropped from MHTML output");

  let uri = contentBase + "file_persist_simple.html";
  let mhtmlFile = await saveAsMHTML("noscript_test", uri);
  ...
  ok(
    ...
    "MHTML should not contain noscript elements"
  );
  ok(
    !content.includes("JavaScript is disabled"),
    "MHTML should not contain noscript content"
  );

  ok(content.includes("<script"), "MHTML should still contain script elements");
  ok(content.includes("MHTML Test Page"), "MHTML should contain page content");
});
