...
nsresult nsWebBrowserPersist::SaveChannelInternal(nsIChannel* aChannel,
  ...
  // Output streams are opened lazily from OnDataAvailable via MakeOutputStream.
  // MHTML saves skip that path and buffer bytes in CollectMHTMLResource.
  return NS_OK;
}
...
void nsWebBrowserPersist::ExtractCSSResources(const nsCString& aCSS,
  ...
  // Parse @import "<url>"; and @import '<url>'; forms.
  src = aCSS.BeginReading();
  while (src < end) {
    const char* importStart = strstr(src, "@import");
    if (!importStart) {
      break;
    }
    ...
    if (*importURL == '"' || *importURL == '\'') {
      char quote = *importURL;
      importURL++;
      const char* importEnd = importURL;
      while (importEnd < end && *importEnd != quote) {
        importEnd++;
      }

      if (importEnd > importURL) {
        nsCString url(Substring(importURL, importEnd));
        url.Trim(" \t\n\r");
        if (!url.IsEmpty() && !StringBeginsWith(url, "data:"_ns) &&
            !aURLs.Contains(url)) {
          aURLs.AppendElement(url);
...
nsresult nsWebBrowserPersist::SerializeAsMHTML() {
  ...
  for (DocData* docData : mDocList) {
    ...
    // For MHTML, we must NOT include noscript content since we assume
    // JavaScript is enabled
    uint32_t encodingFlags =
        mEncodingFlags & ~nsIWebBrowserPersist::ENCODE_FLAGS_NOSCRIPT_CONTENT;
    ...
    // Chrome strips <script> and <noscript> tags from MHTML to prevent
    // execution issues
    ...
    while (src < end) {
      ...
      bool isScript =
          (tagStart + 7 < end && strncasecmp(tagStart, "<script", 7) == 0 &&
           (tagStart[7] == '>' || tagStart[7] == ' ' || tagStart[7] == '\t' ||
            tagStart[7] == '\n'));
      bool isNoScript =
          (tagStart + 9 < end && strncasecmp(tagStart, "<noscript", 9) == 0 &&
           (tagStart[9] == '>' || tagStart[9] == ' ' || tagStart[9] == '\t' ||
            tagStart[9] == '\n'));
