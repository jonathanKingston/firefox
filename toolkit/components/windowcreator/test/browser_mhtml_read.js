...
`;
  ...
  Assert.greaterOrEqual(parts.length, 2, "Should have at least 2 parts");
...
`;
  ...
  Assert.greater(parts.length, 0, "Should parse parts");
  ...
  ok(
    decoded.includes("Hello World!"),
    "Should decode quoted-printable correctly"
  );
});
...
`;
  ...
  Assert.greaterOrEqual(stats.totalParts, 2, "Should have multiple parts");
  Assert.greaterOrEqual(stats.resources, 2, "Should have multiple resources");

  info(
    `Archive has ${stats.totalParts} parts and ${stats.resources} resources`
  );
});
...
add_task(async function test_mhtml_roundtrip() {
  ...
  let mhtmlFile = await BrowserTestUtils.withNewTab(
    uri,
  ...
  // Test detection
  ok(
    MHTMLArchive.isMHTML(mhtmlContent),
    "Saved file should be detected as MHTML"
  );
  ...
  // Scripts and noscripts may be present in saved MHTML content,
  // but scripts should not execute and noscript should not render when loaded.
  // That behavior is tested in browser_mhtml_security.js.

  // Test resource access
  let stats = archive.getStats();
  Assert.greater(stats.totalParts, 0, "Should have parts");
  Assert.greaterOrEqual(stats.resources, 0, "Should have resources map");

  info(
    `Successfully created archive from saved MHTML with ${stats.totalParts} parts`
  );
});
